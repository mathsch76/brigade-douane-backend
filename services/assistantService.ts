...
/**
 * Récupère ou crée un thread avec préférences (version optimisée)
 */
export async function getQuickThread(userId: string, chatbotId: string): Promise<CachedThreadData> {
  ...
  try {
    ...
    if (threadData.data && (Date.now() - new Date(threadData.data.last_used_at).getTime()) < 3600000) {
      let threadId = threadData.data.thread_id;
      ...
      threadId = thread.id;
if (!threadId) {
  console.error("❌ ERREUR - threadId est encore undefined après la création du thread !");
  throw new Error("Impossible de créer un thread valide. threadId manquant.");
}
